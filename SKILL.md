...
Inputs can be local code files/directories or remote GitHub/GitLab repository URLs. For remote repositories, first build a lightweight variable-derivation index and remote code chunks with `scripts/index_remote_repo.py`; then use `remote_code_chunks.jsonl` as the direct input for deeper lineage extraction.

For Codex skill execution, `lineage_facts.jsonl` and `variable_lineage_dictionary.csv` are generated by the LLM workflow from `code_chunks.jsonl` or `remote_code_chunks.jsonl`; they are not produced by standalone Python scripts.
...
1. **Scan and chunk code**
   - If the input is a GitHub/GitLab URL, run `scripts/index_remote_repo.py` with `--chunks-out` to create `output/derivation_index.json` and `output/remote_code_chunks.jsonl`.
   - Remote candidate chunking includes one-hop dependency context by default: Python imports, explicit SQL/config path references, and sibling helper files. Use `--no-include-dependencies` only when the user explicitly wants candidate files alone.
   - Use `derivation_index.json` to inspect candidate feature-generation files and `remote_code_chunks.jsonl` for deeper LLM extraction.
   - If the input is a local file or directory, run `scripts/scan_codebase.py` to create `output/code_chunks.jsonl`.
...
3. **Discover variables and generate lineage facts**
   ...
   - Recognize variable derivation patterns such as:
     - Pandas/dataframe assignments: `df["x"] = ...`, `out[f"x_{w}d"] = ...`, `features["x"] = ...`, `.assign(x=...)`, `.rename(columns={...})`.
     - Spark/PySpark expressions: `.withColumn("x", ...)`, `selectExpr("... AS x")`, `expr("... AS x")`, window specs, `partitionBy`, and chained transformations.
     - Polars-style expressions: `pl.col(...).alias("x")`, `.with_columns(...)`, grouped aggregations with aliases.
     - Aggregation outputs: `groupby(...).agg(x=("col", "sum"))`, `pd.NamedAgg`, pivot/crosstab outputs, rolling/expanding/EWM windows.
     - Dict and return patterns: `features.update({...})`, `return {"x": ...}`, dict comprehensions, feature maps, and template keys such as `f"x_{w}d"`.
     - Config-driven variables: YAML/JSON/TOML/CSV entries such as `feature`, `feature_name`, `variable`, `alias`, `output_name`, feature lists, and loop-expanded configs.
     - SQL-derived variables: `SELECT ... AS x`, CTE aliases, window functions, `CASE WHEN`, `CREATE TABLE AS`, `INSERT INTO`, Jinja/macro-generated SQL, and templated aliases.
     - Model/input assembly patterns: feature column lists, `X = df[feature_cols]`, training/inference feature sets, schema/mapping files, and column allowlists.
     - Upstream preparation patterns: joins, filters, deduplication, date windows, intermediate fields, reusable helper functions, and source table extraction.
...
For remote GitHub/GitLab inputs, use `output/remote_code_chunks.jsonl` in place of `output/code_chunks.jsonl`:
...
Index a remote repository without cloning:

```bash
python3 scripts/index_remote_repo.py https://github.com/org/repo --out output/derivation_index.json --chunks-out output/remote_code_chunks.jsonl
python3 scripts/index_remote_repo.py https://gitlab.com/group/project --provider gitlab --out output/derivation_index.json --chunks-out output/remote_code_chunks.jsonl
```

Remote chunking dependency controls:

```bash
--include-dependencies
--dependency-depth 1
--max-dependency-files 100
